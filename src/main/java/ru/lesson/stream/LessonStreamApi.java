...
public class LessonStreamApi {
    ...
    public List<Employee> task1(List<Employee> employees) {
        ...
        //    List<Employee> list1 = employees.get().getRating().stream().filter(x)
    ...
    //distinctBy нет, пришлось переопределить equals
    public List<Employee> task4(List<List<Employee>> employeeDepartments) {
        return employeeDepartments.stream().flatMap(Collection::stream)
                .distinct()
    ...
    public Map<PositionType, Double> task8(List<Employee> employees) {
        return employees.stream()
                .collect(Collectors.groupingBy(Employee::getPositionType,Collectors
    ...
    public Map<Boolean, Long> task9(List<Employee> employees) {
        return employees.stream().collect(Collectors.groupingBy(o -> o.getRating() > 50, Collectors.counting()));
    }
    ...
    public Map<Boolean, String> task10(List<Employee> employees) {
        return employees.stream()
                .collect(Collectors.groupingBy(o -> o.getRating() > 50, Collectors.mapping(Employee::getName, Collectors.joining(", "))));
    }
